...
DB 설계, 스키마, 인덱스, 쿼리, 트랜잭션, 락, 백업, 캐시, 메시지, ORM, 마이그레이션, 운영 DDL, 대용량 데이터, 장애 대응, 기술 선택과 DB 직무 성장을 다루는 지식 베이스 스킬이다. 72개 문서가 12개 섹션으로 구성되어 있고 질문에 맞는 문서를 라우팅해 출처(`DBA-XXX`)와 함께 답하게 한다.

## 원문 출처

이 지식 베이스의 원문은 Threads에서 활동하는 BEAR DBA(@bear_dba)의 게시글에서 가져왔다.

- 저자: BEAR DBA
- 게시처: https://www.threads.com/@bear_dba
- 허가 범위: 개인 skills 저장소에서의 활용, 수익화 금지
- 원문 8개는 `raw/`에 계보 자료로 보존하며, `annotated/` 문서의 `source_sections`가 원문 범위를 가리킨다.
...
문서 ID는 `DBA-001`부터 `DBA-071`, 그리고 직무 성장 문서 `DBA-999`이며 `document-index.md`와 1:1이다.
