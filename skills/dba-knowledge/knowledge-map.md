...
55개 문서를 12개 지식 섹션으로 묶은 지도다. 각 문서는 주 라우팅 섹션에 한 번만 둔다. 교차 주제는 문서 frontmatter의 `read_also`로 연결한다.
...
라우팅 키워드: NULL, FK, JSON 컬럼, collation, charset, 기본키, 자연키, 인조키, UUID, ULID.
...
라우팅 키워드: DBMS 선택, SQL, NoSQL, MySQL, PostgreSQL, Cassandra, RocksDB, Supabase, MongoDB, 문서 모델.
...
라우팅 키워드: 인덱스, B-tree, 커버링, EXPLAIN, 옵티마이저, 통계, 대용량 조회, OFFSET, keyset, 쿼리 분리.
...
라우팅 키워드: 대량 DELETE, 백필, 파티셔닝, OLAP, 웨어하우스, Athena, Snowflake, BigQuery, 컬럼형 저장.
...
라우팅 키워드: 트랜잭션, 격리 수준, MVCC, 스냅샷, Phantom Read, 락, 데드락, FOR UPDATE, Advisory Lock.
...
라우팅 키워드: JPA, MyBatis, ORM, N+1, Lazy Loading, SQL 매퍼, 프로시저, 커넥션 풀, HikariCP, JSON 계약.
...
라우팅 키워드: ALTER TABLE, 마이그레이션, Rolling Update, Online DDL, gh-ost, pt-osc, soft delete, hard delete, 보존 정책.
...
라우팅 키워드: UNDO, REDO, WAL, Binlog, CDC, 복제, 페일오버, split brain, quorum, fencing, Multi-AZ, Aurora.
...
라우팅 키워드: 백업, 복구, PITR, RPO, RTO, 장애, runbook, wait event, OOM, 메모리, Load Average, AAS, Database Insights.
...
라우팅 키워드: 검색, 결제, 정산, 홈페이지, 쇼핑몰, 주문, 환불, 재고, SNS, 피드, 푸시, 배치, 크롤링, 로그인, 세션.
